...
class PriceWatchSupabaseUser extends BaseAuthUser {
  PriceWatchSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> priceWatchSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = PriceWatchSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
